...
<div class="p-8 space-y-6">
    ...
    <div x-data="bulkSelect('orders')">
        @include('admin.partials._bulk_toolbar', ['module' => 'orders'])
            ...
            <table class="w-full">
                    ...
                    <tr>
                        <th class="px-4 py-3 w-8">
                            <input type="checkbox" x-model="allChecked"
                                   @change="toggleAll([{{ $orders->pluck('id')->join(',') }}])"
                                   class="rounded accent-primary">
                        </th>
                                                @include('admin.partials._sort_th', ['thCol' => 'order_number', 'thLabel' => 'Número'])
                        ...
                        @include('admin.partials._sort_th', ['thCol' => 'total',       'thLabel' => 'Total', 'thAlign' => 'right'])
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-50 transition-colors" :class="selected.includes({{ $order->id }}) ? 'bg-primary/5' : ''">
                            <td class="px-4 py-4">
                                <input type="checkbox" :value="{{ $order->id }}" x-model="selected" class="rounded accent-primary">
                            </td>
