...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="mb-8 flex items-center gap-4">
        <a href="{{ route('admin.categories.index') }}" class="text-gray-400 hover:text-primary transition-colors">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
        </a>
        <h1 class="font-alumni text-h4 text-dark">Editar Categoria: {{ $category->getTranslation('name', 'ca') }}</h1>
    </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 sm:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name[ca]" value="{{ old('name.ca', $category->getTranslation('name', 'ca', false)) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name[es]" value="{{ old('name.es', $category->getTranslation('name', 'es', false)) }}"
                           class="w-full border border-gray-200 rounded-xl px-4 py-2 font-outfit text-sm
                                  focus:outline-none focus:ring-2 focus:ring-primary">
                </div>
                <div>
                    <label class="block font-outfit text-sm font-medium text-gray-700 mb-2">Nom (Anglès)</label>
                    <input type="text" name="name[en]" value="{{ old('name.en', $category->getTranslation('name', 'en', false)) }}"
                           class="w-full border border-gray-200 rounded-xl px-4 py-2 font-outfit text-sm
                                  focus:outline-none focus:ring-2 focus:ring-primary">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description[ca]" rows="3"
                          class="w-full border border-gray-200 rounded-xl px-4 py-2 font-outfit text-sm
                                 focus:outline-none focus:ring-2 focus:ring-primary">{{ old('description.ca', $category->getTranslation('description', 'ca', false)) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="description[es]" rows="3"
                          class="w-full border border-gray-200 rounded-xl px-4 py-2 font-outfit text-sm
                                 focus:outline-none focus:ring-2 focus:ring-primary">{{ old('description.es', $category->getTranslation('description', 'es', false)) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="description[en]" rows="3"
                          class="w-full border border-gray-200 rounded-xl px-4 py-2 font-outfit text-sm
                                 focus:outline-none focus:ring-2 focus:ring-primary">{{ old('description.en', $category->getTranslation('description', 'en', false)) }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block font-outfit text-sm font-medium text-gray-700 mb-2">Categoria Pare</label>
                    <select name="parent_id" class="w-full border border-gray-200 rounded-xl px-4 py-2 font-outfit text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                        <option value="">Cap (Categoria Principal)</option>
                        @foreach($parents as $parent)
                            <option value="{{ $parent->id }}" @selected(old('parent_id', $category->parent_id) == $parent->id)>
                                {{ $parent->getTranslation('name', 'ca') }}
                            </option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <input type="number" name="sort_order" value="{{ old('sort_order', $category->sort_order) }}" required min="0"
                           class="w-full border border-gray-200 rounded-xl px-4 py-2 font-outfit text-sm
                                  focus:outline-none focus:ring-2 focus:ring-primary">
            ...
            <div>
                ...
                @if($category->image)
                    <div class="mb-4">
                        <img src="{{ asset('storage/'.$category->image) }}" class="w-32 h-32 object-cover rounded-xl border border-gray-200">
                    </div>
                @endif
            ...
            <div class="flex items-center gap-3">
                ...
                <input type="checkbox" name="is_active" id="is_active" value="1" @checked(old('is_active', $category->is_active))
                       class="w-5 h-5 text-primary border-gray-300 rounded focus:ring-primary">
                <label for="is_active" class="font-outfit text-sm text-gray-700">Categoria Activa</label>
            </div>
        </div>
